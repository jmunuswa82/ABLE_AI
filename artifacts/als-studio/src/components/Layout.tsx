...
import {
  LayoutDashboard, FileArchive, Activity, GitCommitHorizontal,
  Layers, Hexagon, Menu, X,
} from "lucide-react";
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  const sidebarContent = (
    <>
      <div className="px-6 py-6 md:py-8 border-b border-[var(--amber-border)] relative overflow-hidden">
      ...
      <nav className="flex-1 py-4 md:py-6 px-4 space-y-1 overflow-y-auto">
          ...
          <AnimatePresence>
            {projectNavItems.length > 0 && (
              <motion.div
                ...
                className="pt-4 md:pt-6 overflow-hidden"
              >
                <div className="px-3 pb-3">
      ...
      <div className="px-6 py-4 md:py-6 border-t border-[var(--amber-border)] bg-[var(--bg-card)]">
  ...
  return (
    <div className="flex h-screen bg-[var(--bg-base)] text-[var(--text-primary)] overflow-hidden selection:bg-primary/30">
      ...
      {isMobile ? (
        <>
          <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
            <SheetContent
              side="left"
              className="w-64 p-0 bg-[var(--bg-card)] border-r border-[var(--amber-border-strong)] flex flex-col"
            >
              {sidebarContent}
            </SheetContent>
          </Sheet>
      ...
      <div className="flex-1 flex flex-col relative min-w-0">
        {isMobile && (
          <header className="h-12 bg-[var(--bg-card)] border-b border-[var(--amber-border)] flex items-center px-4 z-30 shrink-0">
            <button
            ...
            <div className="flex items-center gap-2 ml-3">
              ...
              <span className="text-[10px] font-label text-[var(--text-secondary)] uppercase tracking-widest">
                ALS Studio
              </span>
            </div>
          </header>
        ...
        <footer className="h-[32px] bg-[var(--bg-card)] border-t border-[rgba(81,69,50,0.05)] flex items-center justify-between px-4 md:px-6 z-30 shrink-0">
          <div className="flex items-center gap-4 md:gap-6 text-[8px] md:text-[9px] font-label uppercase tracking-[1.8px] text-[var(--text-footer)]">
            <div className="flex items-center gap-2">
              <span className="relative flex h-1.5 w-1.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#22c55e] opacity-75" />
                <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-[#22c55e]" />
              </span>
              <span className="hidden sm:inline">Server: Amsterdam-01</span>
            </div>
            <span className="hidden sm:inline">Latency: 24ms</span>
          </div>
          <div className="flex items-center gap-4 md:gap-6 text-[8px] md:text-[9px] font-label uppercase tracking-[1.8px] text-[var(--text-footer)]">
            <span className="hidden sm:inline">ALS STUDIO © 2024</span>
